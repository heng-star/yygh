...
export default {
    ...
    created() {
        ...
        hospitalApi.findByDictCode('86').then(response => {
    ...
    methods: {
        ...
        fetchData(page = 1) {
            ...
            hospitalApi.getPageList(this.page, this.limit, this.searchObj).then(
                response => {
                this.list = response.data.content  //hospitalController里的参数
                this.total = response.data.totalElements  //hospitalController里的参数
        ...
        updateStatus(id, status) {
             hospitalApi.updateStatusbyid(id, status)
            .then(response => { this.fetchData(this.page)})
        },
